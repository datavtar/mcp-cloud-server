...
class GeminiProvider(LLMProvider):
    ...
    def __init__(self):
        # Try Vertex AI key first (GOOGLE_CLOUD_API_KEY), fall back to regular key
        api_key = os.environ.get("GOOGLE_CLOUD_API_KEY") or os.environ.get("GOOGLE_API_KEY")
        if not api_key:
            raise ValueError("GOOGLE_CLOUD_API_KEY or GOOGLE_API_KEY environment variable is required for Gemini provider")

        # Use Vertex AI mode if GOOGLE_CLOUD_API_KEY is set
        use_vertex = os.environ.get("GOOGLE_CLOUD_API_KEY") is not None
        self.client = genai.Client(api_key=api_key, vertexai=use_vertex)
